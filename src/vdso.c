 ...
 *
 *   __kernel_rt_sigreturn    (NR 139) — SVC trampoline (signal restorer)
 *   __kernel_clock_getres    (NR 114) — SVC trampoline (rare)
 *   __kernel_clock_gettime   (NR 113) — real: seqlock-read [vvar]
 *   __kernel_gettimeofday    (NR 169) — real: seqlock-read [vvar]
 *
 * Time helpers read host-published timestamps from [vvar] at VVAR_BASE
 * (see hl_vvar_t / vdso_vvar_update). rt_sigreturn stays an SVC trampoline
 * at a fixed offset (syscall_signal restorer = VDSO_BASE + VDSO_OFF_TEXT).
 * Guest .text is assembled from src/vdso_guest.S → _build/vdso_guest_bin.h.
 *
 ...
 *   0x2B0   .dynamic             96 bytes (6 entries × 16)
 *   0x310   Shdr[0..5]           384 bytes (6 × 64)
 *   0x490   end (within 4KB page)
 *
 * [vvar] lives on a separate page at VVAR_BASE (0xE000), host-written.
 */
#include "vdso.h"
#include "elf.h"
...
/* Generated by Makefile from src/vdso_guest.S */
#include "vdso_guest_bin.h"
...
/* DYNSYM = 5 × 24 = 120 bytes → ends at 0x288 (already 8-byte aligned) */
#define VDSO_OFF_HASH    0x288   /* after .dynsym */
/* HASH = 4+4+4+20 = 32 bytes → ends at 0x2A8, round up to 8-byte: 0x2B0 */
#define VDSO_OFF_DYNAMIC 0x2B0   /* after .hash */
/* DYNAMIC = 6 × 16 = 96 bytes → ends at 0x310 */
#define VDSO_OFF_SHDR    0x310   /* after .dynamic */
/* SHDR = 6 × 64 = 384 bytes → ends at 0x490 (within 4KB page) */
...
/* .text: assembled guest blob (src/vdso_guest.S). Offsets within the blob
 * are VDSO_SYM_* in vdso.h (must match nm -n on vdso_guest.o):
...
static const uint32_t sym_text_off[VDSO_NUM_SYMS] = {
    ...
    VDSO_SYM_CLOCK_GETRES,
    VDSO_SYM_CLOCK_GETTIME,
    VDSO_SYM_GETTIMEOFDAY,
};
static const uint32_t sym_text_size[VDSO_NUM_SYMS] = {
    VDSO_SYM_CLOCK_GETRES - VDSO_SYM_RT_SIGRETURN,
    VDSO_SYM_CLOCK_GETTIME - VDSO_SYM_CLOCK_GETRES,
    VDSO_SYM_GETTIMEOFDAY - VDSO_SYM_CLOCK_GETTIME,
    VDSO_GUEST_TEXT_SIZE - VDSO_SYM_GETTIMEOFDAY,
};
...
/* ---------- [vvar] host publisher ----------
 * Guest vDSO reads REALTIME/MONOTONIC fields under a seqlock. Host writes
 * via host_base (guest PTEs are RX on the low 2MB block — no EL0 store).
 * A 1ms publisher thread covers pure gettimeofday loops with no other
 * syscalls (e.g. GTK idle). */

static guest_t *pub_guest;
static pthread_t pub_thread;
static atomic_int pub_run;
static int pub_started;

void vdso_vvar_update(guest_t *g) {
    if (!g || !g->host_base) return;
    if (VVAR_BASE + sizeof(hl_vvar_t) > g->guest_size) return;

    hl_vvar_t *vv = (hl_vvar_t *)((uint8_t *)g->host_base + VVAR_BASE);

    struct timespec rt, mono;
    if (clock_gettime(CLOCK_REALTIME, &rt) != 0) return;
    if (clock_gettime(CLOCK_MONOTONIC, &mono) != 0) return;

    /* seqlock: odd while writing, even when stable */
    (void)__atomic_fetch_add(&vv->seq, 1u, __ATOMIC_RELEASE);

    vv->version = HL_VVAR_VERSION;
    vv->realtime_sec = rt.tv_sec;
    vv->realtime_nsec = rt.tv_nsec;
    vv->mono_sec = mono.tv_sec;
    vv->mono_nsec = mono.tv_nsec;

    (void)__atomic_fetch_add(&vv->seq, 1u, __ATOMIC_RELEASE);
}

static void *vdso_publisher_main(void *arg) {
    (void)arg;
    while (atomic_load(&pub_run)) {
        guest_t *g = pub_guest;
        if (g)
            vdso_vvar_update(g);
...
void vdso_publisher_start(guest_t *g) {
    if (!g) return;
    pub_guest = g;
    vdso_vvar_update(g); /* first sample immediately */
    if (pub_started) return;
    atomic_store(&pub_run, 1);
    if (pthread_create(&pub_thread, NULL, vdso_publisher_main, NULL) != 0) {
        atomic_store(&pub_run, 0);
        fprintf(stderr, "hl: vdso publisher thread failed\n");
        return;
    }
    pub_started = 1;
}
...
uint64_t vdso_build(guest_t *g) {
    if (vdso_guest_bin_len < VDSO_GUEST_TEXT_SIZE) {
        fprintf(stderr, "vdso: guest text blob too small (%u < %u)\n",
                (unsigned)vdso_guest_bin_len, (unsigned)VDSO_GUEST_TEXT_SIZE);
        return 0;
    }
    if (VDSO_OFF_TEXT + VDSO_GUEST_TEXT_SIZE > VDSO_SIZE) {
        fprintf(stderr, "vdso: text does not fit in page\n");
        return 0;
    }
    ...
    /* Clear [vvar]; version stays 0 until first publish (guest SVC fallback) */
    if (guest_ptr(g, VVAR_BASE)) {
        memset((uint8_t *)g->host_base + VVAR_BASE, 0, sizeof(hl_vvar_t));
    }
    ...
    /* ---- .text: guest blob (trampolines + real time helpers) ---- */
    memcpy(page + VDSO_OFF_TEXT, vdso_guest_bin, VDSO_GUEST_TEXT_SIZE);
    ...
    for (int i = 0; i < VDSO_NUM_SYMS; i++) {
        ...
        sym[i + 1].st_value = VDSO_OFF_TEXT + sym_text_off[i];
    ...
    shdr[1].sh_size      = VDSO_GUEST_TEXT_SIZE;
    shdr[1].sh_link      = 0;
    shdr[1].sh_info      = 0;
    ...
    /* Publish time so version==1 before the guest can call vDSO helpers */
    vdso_vvar_update(g);
