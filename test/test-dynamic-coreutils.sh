#!/usr/bin/env bash
# test-dynamic-coreutils.sh — Dynamically-linked GNU coreutils test suite for hl
#
# Copyright 2025 Moritz Angermann <moritz@zw3rk.com>, zw3rk pte. ltd.
# SPDX-License-Identifier: Apache-2.0
#
# Mirrors test-coreutils.sh but invokes every tool through hl --sysroot,
# exercising the dynamic linker (ld-musl-aarch64.so.1) and shared libc.so.
#
# Usage: test/test-dynamic-coreutils.sh <hl-binary> <sysroot-dir> <coreutils-bin-dir>
# Example: test/test-dynamic-coreutils.sh _build/hl $GUEST_SYSROOT $GUEST_DYNAMIC_COREUTILS/bin
...
printf "\n${BLUE}━━━ Dynamic GNU coreutils test suite (--sysroot) ━━━${RESET}\n\n"
...
printf "\n${BLUE}━━━ Dynamic results: %d passed, %d failed, %d xfail, %d skipped (of %d) ━━━${RESET}\n" \
