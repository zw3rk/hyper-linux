#!/usr/bin/env bash
# test-coreutils.sh — Automated GNU coreutils 9.9 test suite for hl
#
# Copyright 2025 Moritz Angermann <moritz@zw3rk.com>, zw3rk pte. ltd.
# SPDX-License-Identifier: Apache-2.0
#
# Tests all 104 coreutils through hl, verifying each tool can at least
# run and produce expected output or exit code. Each test is designed to
# exercise the tool's core functionality without requiring network, users
# database, or signal delivery.
#
# Usage: test/test-coreutils.sh <hl-binary> <coreutils-bin-dir>
# Example: test/test-coreutils.sh _build/hl /nix/store/.../bin

set -euo pipefail

HL="${1:?Usage: $0 <hl-binary> <coreutils-bin-dir>}"
BIN="${2:?Usage: $0 <hl-binary> <coreutils-bin-dir>}"
...
# Run a coreutils tool through hl. Args:
#   $1 = tool name
...
run() {
    ...
    if output=$("$HL" "$BIN/$tool" "$@" 2>&1); then
        rc=0
    else
        rc=$?
    fi
...
run_timeout() {
    ...
    if output=$(timeout "$secs" "$HL" "$BIN/$tool" "$@" 2>&1); then
        rc=0
    else
        rc=$?
    fi
...
printf "\n${BLUE}━━━ GNU coreutils 9.9 test suite ━━━${RESET}\n\n"
...
run_skip   stdbuf    "requires LD_PRELOAD (N/A for static)"
run_skip   chroot    "needs root privileges"

# ── Summary ──────────────────────────────────────────────────────
total=$((pass + fail + expected_fail + skip))
printf "\n${BLUE}━━━ Results: %d passed, %d failed, %d xfail, %d skipped (of %d) ━━━${RESET}\n" \
