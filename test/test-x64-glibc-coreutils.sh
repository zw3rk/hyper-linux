#!/usr/bin/env bash
# test-x64-glibc-coreutils.sh — x86_64 glibc dynamically-linked GNU coreutils test suite for hl
#
# Copyright 2025 Moritz Angermann <moritz@zw3rk.com>, zw3rk pte. ltd.
# SPDX-License-Identifier: Apache-2.0
#
# Tests x86_64 glibc coreutils via rosetta, invokes every tool through hl --sysroot,
# exercising the dynamic linker (ld-linux-x86-64.so.2) and glibc shared libc.so.6 via Rosetta.
#
# Usage: test/test-x64-glibc-coreutils.sh <hl-binary> <sysroot-dir> <coreutils-bin-dir>
# Example: test/test-x64-glibc-coreutils.sh _build/hl $GUEST_SYSROOT $GUEST_DYNAMIC_COREUTILS/bin
...
printf "\n${BLUE}━━━ x86_64 glibc dynamic GNU coreutils (Rosetta) test suite (--sysroot) ━━━${RESET}\n\n"
...
printf "\n${BLUE}━━━ x86_64 glibc results: %d passed, %d failed, %d xfail, %d skipped (of %d) ━━━${RESET}\n" \
